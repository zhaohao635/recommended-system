package com.atiguigu.streaming
...
class StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //继续做流式处理，核心算法部分
    ratingStream.foreachRDD{    //每一个值其实是一组RDD
      rdds => rdds.foreach{
  ...
  def computeMovieScores(candidateMovies: Array[Int],
    ...
    //根据备选电影的mid做groupby根据公式去求最后的推荐评分
  ...
  def getMoviesSimScore(mid1: Int, mid2: Int, simMovies:  scala.collection.Map[Int,
    ...
    simMovies.get(mid1) match {
      case Some(sims) => sims.get(mid2) match {   //有值的话，就进行匹配
