package com.atiguigu.statistics
...
//
    //3.优质电影统计，统计电影的平均分，mid,avg
    val averageMoviesDF = spark.sql("select mid,avg(score) as avg from ratings group by mid")
    averageMoviesDF.show(truncate = false)
...
//
    ...
    val genresTopMoviesDF = genresRDD.cartesian(movieWithScore.rdd)
        .filter{
          //条件过滤，找出movie的字段genres值（Adventure|Animation|Children|Comedy|Fantasy）包含当前类别genre的那些
      ...
      //case class GenresRecommendation(genres:String,recs:Seq[Recommendation])
        ...
        .toDF()
//    genresTopMoviesDF.show(truncate = false)
